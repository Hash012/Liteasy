    ...
    <div className="admin-view visualization-governance">
      ...
      {!loading ? <>
        ...
        {routeDraft ? <section className="admin-section visualization-advanced"><h2>高级管理员 Provider 配置</h2><form className="admin-form admin-form-horizontal visualization-editor" onSubmit={saveRoute}>
          ...
          <Field label="超时（毫秒）" required><Input aria-label="超时（毫秒）" name="timeoutMs" defaultValue={String(routeDraft.timeoutMs)} max={600000} min={100} type="number" /></Field><Field label="优先级"><Input aria-label="优先级" name="priority" defaultValue={String(routeDraft.priority)} min={0} type="number" /></Field>

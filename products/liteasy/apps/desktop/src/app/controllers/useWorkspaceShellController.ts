...
export function readingFileStatus(entry?: ReadingCatalogEntry): FileStatus | undefined {
...
export function useWorkspaceShellController(input: {
  ...
  function search() {
    ...
    const label = {
      library: "搜索文献资源", notes: "搜索笔记", pdf: "搜索文档内容",
      "reading-catalog": "搜索文献与文件", "reading-document": "搜索书内文字"
    }[active.search];
    const field = region?.querySelector<HTMLInputElement>(`input[aria-label="${label}"]`);
    if (field) { field.focus(); field.select(); }
    else if (active.search === "reading-document") region?.querySelector<HTMLButtonElement>('button[aria-label="搜索正文"]')?.click();

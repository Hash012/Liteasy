...
type Species = ReactionProcessSpecV1["species"][number];
type StoichiometricItem = ReactionStep["reactants"][number];
type Rational = { denominator: bigint; numerator: bigint };
...
export function parseChemicalSpeciesFormula(formula: string): ParsedChemicalFormula {
  if (formula.length === 0 || formula.length > maxFormulaLength || formula.trim() !== formula || /\s/u.test(formula)) {
  ...
  for (const segment of formulaBody.split(/[.·•]/u)) {
    ...
    const parsed = parseFormulaSequence(body, 0, undefined);
...
export function balanceReaction(step: ReactionStep, species: readonly Species[]): Record<string, number> {
  ...
  const matrix = [...elements, "__charge__"].map((element) => items.map((_, columnIndex) => {
    ...
    const value = element === "__charge__"
      ? parsed[columnIndex].charge
      : parsed[columnIndex].atoms[element] ?? 0;
...
export function validateReactionProcess(spec: ReactionProcessSpecV1): ReactionProcessResultV1 {
  ...
  if (speciesById.size !== spec.species.length || speciesById.size === 0) throw new Error("reaction_species_duplicate");
  ...
  for (const step of spec.steps) {
    ...
    assertMinimalCoefficients(step, balanceReaction(step, spec.species));
    equations.push({ id: step.id, text: equationText(step, speciesById) });
  }
  if (equations.length === 0) throw new Error("reaction_step_missing");
  for (const condition of spec.conditions) {
    if (condition.evidenceClaimIds.length === 0) throw new Error("reaction_condition_unbound");
  }
  validateAtomMap(spec, speciesById);

  const selectableObjectIds = [...spec.species.map((species) => species.id), ...spec.steps.map((step) => step.id)];
  return {
    accessibility: {
      dataTable: [
        ...equations.map((equation) => ({ label: equation.id, value: equation.text })),
        ...spec.conditions.map((condition) => ({ label: condition.label, value: condition.value ?? "present" }))
      ],
...
function parseFormulaSequence(body: string, startIndex: number, closing: string | undefined): {
  atomOrder: string[];
  atoms: FormulaCounts;
  nextIndex: number;
} {
  const depth = closing ? 1 : 0;
  return parseAtDepth(body, startIndex, closing, depth);
}

function parseAtDepth(body: string, startIndex: number, closing: string | undefined, depth: number): {
  atomOrder: string[];
  atoms: FormulaCounts;
  nextIndex: number;
} {
...
function splitCharge(formula: string): { charge: number; formulaBody: string } {
  const caret = formula.match(/\^(\d*)([+-])$/u);
  if (caret) {
    return {
      charge: signedCharge(caret[1], caret[2]),
      formulaBody: formula.slice(0, caret.index)
    };
  }
  const signThenMagnitude = formula.match(/([+-])(\d+)$/u);
  if (signThenMagnitude) {
    return {
      charge: signedCharge(signThenMagnitude[2], signThenMagnitude[1]),
      formulaBody: formula.slice(0, signThenMagnitude.index)
    };
  }
...
function signedCharge(magnitudeText: string, sign: string): number {
  const magnitude = magnitudeText ? boundedPositiveInteger(magnitudeText) : 1;
  return sign === "+" ? magnitude : -magnitude;
}

function readNumber(body: string, startIndex: number): { nextIndex: number; value: number } {
  let index = startIndex;
...
function boundedPositiveInteger(value: string): number {
...
function mergeCounts(target: FormulaCounts, source: Readonly<FormulaCounts>, multiplier: number): void {
...
function appendAtoms(target: string[], source: readonly string[], multiplier: number): void {
...
function assertReferences(step: ReactionStep, speciesById: ReadonlyMap<string, Species>): void {
  if (step.reactants.length === 0 || step.products.length === 0) throw new Error("reaction_reference_invalid");
  const sideIds = [new Set<string>(), new Set<string>()];
  for (const [sideIndex, items] of [step.reactants, step.products].entries()) {
    for (const item of items) {
      if (!speciesById.has(item.speciesId) || !Number.isInteger(item.coefficient) || item.coefficient <= 0 || item.coefficient > maxCoefficient || sideIds[sideIndex].has(item.speciesId)) {
...
function assertConserved(step: ReactionStep, speciesById: ReadonlyMap<string, Species>): void {
...
function assertMinimalCoefficients(step: ReactionStep, balanced: Readonly<Record<string, number>>): void {
...
function aggregate(items: readonly StoichiometricItem[], speciesById: ReadonlyMap<string, Species>): { atoms: FormulaCounts; charge: number } {
  const atoms: FormulaCounts = {};
  let charge = 0;
  for (const item of items) {
...
function validateAtomMap(spec: ReactionProcessSpecV1, speciesById: ReadonlyMap<string, Species>): void {
  if (spec.atomMap.length === 0) return;
  const mapIds = new Set<string>();
  const entriesByStep = new Map<string, typeof spec.atomMap>();
  for (const entry of spec.atomMap) {
    if (mapIds.has(entry.id)) throw new Error("reaction_atom_map_duplicate");
    mapIds.add(entry.id);
    if (entry.evidenceClaimIds.length === 0) throw new Error("reaction_atom_map_unbound");
    const stepId = entry.stepId ?? (spec.steps.length === 1 ? spec.steps[0].id : undefined);
    if (!stepId || !spec.steps.some((step) => step.id === stepId)) throw new Error("reaction_atom_map_step_invalid");
...
function enumerateSideAtoms(items: readonly StoichiometricItem[], speciesById: ReadonlyMap<string, Species>): Map<string, string> {
  ...
  for (const item of items) {
    ...
    for (let molecule = 0; molecule < item.coefficient; molecule += 1) {
      for (let atom = 0; atom < atomOrder.length; atom += 1) {
        if (atoms.size >= 4096) throw new Error("reaction_atom_map_limit");
...
function resolveMappedAtom(
  speciesId: string,
  requestedMolecule: number | undefined,
  atom: number,
  items: readonly StoichiometricItem[],
  atoms: ReadonlyMap<string, string>
): { element: string; key: string } {
...
function atomKey(speciesId: string, molecule: number, atom: number): string {
  return `${speciesId}:${molecule}:${atom}`;
}

function solveOneDimensionalNullspace(matrix: Rational[][]): number[] {
...
function equationText(step: ReactionStep, speciesById: ReadonlyMap<string, Species>): string {
  const side = (items: readonly StoichiometricItem[]) => items
    .map((item) => `${item.coefficient === 1 ? "" : item.coefficient}${speciesById.get(item.speciesId)!.formula}(${speciesById.get(item.speciesId)!.state})`)
    .join(" + ");

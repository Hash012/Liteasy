...
export function ThinReadingVisualizationRegion({ artifacts, status }: ThinReadingVisualizationRegionProps) {
  ...
  return (
    <section aria-label="生成可视化" className="thin-reading__visualizations" data-testid="thin-reading-visuals">
      {artifacts.length > 0 ? artifacts.map((artifact) => (
        <VisualizationArtifactHost artifact={artifact} key={artifact.artifactId} />
      )) : (

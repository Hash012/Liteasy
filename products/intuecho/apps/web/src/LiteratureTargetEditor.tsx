...
type Props = { onChange: (targets: AnnotationTarget[]) => void; required: boolean; targets: AnnotationTarget[] };
...
export function LiteratureTargetEditor({ onChange, required, targets }: Props) {
  ...
  async function search() {
    ...
    try {
      ...
      setResult(next);
      if (next.status === "exact") await confirm(next.candidate);
      if (next.status === "unavailable") setStatus("文献检索服务暂时不可用，请重试。");
      if (next.status === "not_found") setStatus("没有找到匹配的文献，可手动添加。");
    } catch (reason) { setStatus(reason instanceof Error ? reason.message : "文献检索失败"); }
    finally { setLoading(false); }
  }

  async function confirm(candidate: LiteratureCandidate) {
  ...
  async function addConfirmed(record: LiteratureRecord, selectedExcerpt = excerpt, selectedPage = page) {
    if (kind === "source_passage" && !selectedExcerpt.trim()) { setStatus("请填写原文摘录"); return; }
    onChange([...targets, await confirmedTarget(kind, record.literatureId, selectedExcerpt, selectedPage)]);

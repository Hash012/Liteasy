...
const advisoryLock = "intuecho-schema-migrations-v1";

function quotedIdentifier(value) {
  if (!/^[A-Za-z_][A-Za-z0-9_]{0,62}$/.test(value ?? "")) {
    throw new Error("intuecho_application_role_invalid");
  }
  return `"${value}"`;
}

export function readIntuechoMigrations(directory = defaultDirectory) {
...
export async function migrateIntuecho(pool, { applicationRole, directory = defaultDirectory } = {}) {
  ...
  try {
    ...
    for (const migration of readIntuechoMigrations(directory)) {
      const checksum = existing.get(migration.name);
      if (checksum && checksum !== migration.checksum) {
        throw new Error(`intuecho_migration_changed: ${migration.name}`);
      }
    ...
    if (applicationRole) {
      ...
      await client.query(`REVOKE UPDATE, DELETE ON moderation_audit FROM ${role}`);
      await client.query(`REVOKE UPDATE, DELETE ON annotation_moderation_audit FROM ${role}`);
      await client.query(`REVOKE UPDATE, DELETE ON annotation_tag_appeal_audit FROM ${role}`);
    ...
    try {
      await client.query("SELECT pg_advisory_unlock(hashtext($1))", [advisoryLock]);
    } finally {
      client.release();
    }
  }
...
export async function verifyIntuechoMigrations(pool, { directory = defaultDirectory } = {}) {
  let result;
  try {
    result = await pool.query("SELECT name, checksum_sha256 FROM schema_migrations ORDER BY name");
  } catch (error) {
    if (error?.code === "42P01") throw new Error("intuecho_migrations_missing");
    throw error;
  }
  const applied = new Map(result.rows.map((row) => [row.name, row.checksum_sha256]));
  const expected = readIntuechoMigrations(directory);
  for (const migration of expected) {
    const checksum = applied.get(migration.name);
    if (!checksum) throw new Error(`intuecho_migration_missing: ${migration.name}`);
    if (checksum !== migration.checksum) throw new Error(`intuecho_migration_changed: ${migration.name}`);
  }
  const unknown = [...applied.keys()].filter((name) => !expected.some((item) => item.name === name));
  if (unknown.length > 0) throw new Error(`intuecho_migration_unknown: ${unknown.join(",")}`);

...
export class PostgresForumRepository {
  ...
  async syncCommunityAnnotations(viewerId, items) {
    requiredId(viewerId, "USER_ID_INVALID");
    return withTransaction(this.pool, async (client) => {
      const results = [];
      for (const item of items) {
        const identity = item.paperIdentity?.primary ?? item.scope.paperIdentity?.primary;
        ...
        const saved = await client.query(`
          INSERT INTO community_annotations(
            ...
            paper_identity_value, scope_kind, scope, body, excerpt,
          ...
          RETURNING id
        `, [
          annotationId, viewerId, item.annotationId, item.queueKey, identity.kind,
          identity.value, item.scope.kind, JSON.stringify(item.scope), item.body, item.excerpt,
          item.createdAt, item.updatedAt
        ]);
        const id = saved.rows[0]?.id ?? (await client.query(
          "SELECT id FROM community_annotations WHERE owner_id = $1 AND queue_key = $2",
          [viewerId, item.queueKey]
        )).rows[0].id;
        results.push({
  ...
  async communityRecommendations(scope) {
    const result = await this.pool.query(`
      SELECT id, body, excerpt, paper_identity_kind, paper_identity_value, scope_kind, scope
        FROM community_annotations
       WHERE paper_identity_kind = $1 AND paper_identity_value = $2
       ORDER BY updated_at DESC, id
       LIMIT 20
    `, [scope.paperIdentity.kind, scope.paperIdentity.value]);
    return result.rows.map((row) => {
      const exactSection = scope.kind === "section" && row.scope_kind === "section" &&
        scope.sectionKey && row.scope.sectionKey === scope.sectionKey;
      const exactPassage = scope.kind === "selected_passage" && row.scope_kind === "selected_passage";
      return {

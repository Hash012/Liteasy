...
Intuecho 不预置论坛账号。启动 dev-cloud、论坛 API 和 Web 后，在 `http://127.0.0.1:5174` 的登录页切换到“注册”，建议使用 `qa.<姓名或工号>@liteasy.local` 和由测试人员自行保管的 12–128 位密码。账号由 dev-cloud 保存；同一邮箱可登录 Desktop，但论坛签发的是 `intuecho-web` 会话，不能复用 `liteasy-desktop` token。

正式 OIDC/Keycloak 环境同样不提供仓库固定账号，测试人员须通过 IdP 注册流程创建个人用户。公开广场读取可匿名进行；发布、回复、关注、消息和个人资料测试必须登录。论坛自身不创建管理员，治理测试还要求该用户已在 Liteasy API 获得 `platform_admin` 且完成新鲜 MFA。

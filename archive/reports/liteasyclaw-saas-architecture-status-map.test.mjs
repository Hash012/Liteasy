...
const targetFile = path.join(__dirname, "liteasyclaw-saas-architecture-status-map-2026-05-16.html");
...
test("the SaaS architecture status map report includes the required interactive views", () => {
  ...
  assert.match(html, /LiteasyClaw SaaS 架构全景/);
  assert.match(html, /产品架构视角/);
  assert.match(html, /工程映射视角/);
  assert.match(html, /OpenClaw/);
  assert.match(html, /LiteasyClaw 与 OpenClaw 的关系/);
  ...
  assert.doesNotMatch(html, /既展示理想中的 LiteasyClaw SaaS 应该由哪些层构成/);

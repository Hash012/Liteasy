...
```bash
node /home/octopus/Liteasy/development/dev-cloud/server.mjs
```
...
- `node --test development/dev-cloud/server.test.mjs development/dev-cloud/providers/openaiResponses.test.mjs`：22 个服务端/Provider 测试通过。
- `cd products/liteasy/apps/desktop && npm test`：63 个桌面端测试文件、223 个测试通过。
- `cd products/liteasy/apps/desktop && npm run build`：TypeScript 与 Vite 生产构建通过。

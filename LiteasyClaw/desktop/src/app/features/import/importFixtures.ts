...
import { extractPdfChunksForPaper } from "./pdfTextExtractor";

export function buildImportedChunksForPaper(paper: Paper): RetrievalChunk[] {
  return buildDemoChunksForPaper(paper);
}

export async function extractImportedChunksForPaper(paper: Paper): Promise<RetrievalChunk[]> {
  try {
    return await extractPdfChunksForPaper(paper);
  } catch (error) {

...
node deploy/local/foundation.mjs restart
node deploy/local/foundation.mjs stop
```

Run `prepare`, `start`, `migrate`, and `verify` in that order for a new environment.
`prepare` creates or completes `deploy/local/.env` with mode `0600`; reruns only add
...
`deploy/local/.env.example` is the non-secret schema. Images, bind address, runtime
...
keys, caches, or `deploy/local/.env`. Keycloak, Liteasy, and Intuecho databases and
...
- If startup fails, run `status` and inspect `docker compose --env-file deploy/local/.env --file deploy/local/compose.yaml logs --no-color <service>` before changing configuration.

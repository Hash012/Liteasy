...
export const localDefaults = Object.freeze({
  ...
  LITEASY_DESKTOP_LOOPBACK_REDIRECT_URI: "http://127.0.0.1/oauth/callback",
  LITEASY_DESKTOP_LOCALHOST_REDIRECT_URI: "http://localhost/oauth/callback",
...
export function readLocalEnvironment({ required = true } = {}) {
  if (!fs.existsSync(localEnvPath)) {
    if (required) throw new Error("deployment/local/.env is missing; run node deployment/local/prepare.mjs");
    return {};
  }
  return parseEnvironmentFile(fs.readFileSync(localEnvPath, "utf8"), "deployment/local/.env");
}
...
export function localPort(values, name) {
  ...
  if (!Number.isSafeInteger(port) || port < 1 || port > 65535) {
    throw new Error(`deployment/local/.env contains an invalid ${name}`);
  }
  return port;
}

...
版本：2026-08-07  
适用设计：`project-docs/design/Liteasy-文件系统与存储边界设计.md`  
...
- `deploy/local/.env`、`.env.local`、模型 API key 和本机自签名证书；
...
```bash
node deploy/local/foundation.mjs prepare
node deploy/local/foundation.mjs start
node deploy/local/foundation.mjs migrate
node deploy/local/foundation.mjs verify
node deploy/local/foundation.mjs status
node deploy/local/foundation.mjs stop
```
...
```powershell
node LiteasyClaw/desktop/scripts/windows-tauri-login-check.mjs
node LiteasyClaw/desktop/scripts/windows-tauri-filesystem-acceptance.mjs
node LiteasyClaw/desktop/scripts/windows-tauri-watcher-overflow-acceptance.mjs
node LiteasyClaw/desktop/scripts/windows-tauri-library-ui-acceptance.mjs
```
...
```bash
node deploy/local/foundation.mjs migrate
node deploy/local/verify-postgres.mjs
node deploy/local/verify-intuecho-postgres-integration.mjs
cd Intuecho && npm test && npm run build
cd LiteasyClaw/desktop && npm test -- --run src/tests/thinReadingIntuechoSyncQueue.test.ts src/tests/thinReadingCommunityRecommendationClient.test.ts src/tests/thinReadingTab.test.tsx

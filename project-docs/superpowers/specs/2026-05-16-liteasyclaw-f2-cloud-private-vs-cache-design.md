...
- `LiteasyClaw/desktop/src/app/features/collection/` already has a cloud collection client/runtime path, but `useCollectionItems` still loads and stores collection items through local browser storage.
- `LiteasyClaw/desktop/src/app/features/recommendations/` still treats recommendation cache as a frontend-only in-memory `Map` inside `useRecommendations`.
- `LiteasyClaw/services/dev-cloud/` already exposes:
...
`LiteasyClaw/desktop/src/app/features/collection/` remains the user-cloud-private favorites domain.
...
`LiteasyClaw/desktop/src/app/features/recommendations/` becomes the recommendation generation plus recommendation-cache orchestration domain.
...
`LiteasyClaw/desktop/src/app/features/library/LibraryPane.tsx` keeps the current user-facing section names and drag behavior.
...
`LiteasyClaw/services/dev-cloud/` must separate:
...
- `LiteasyClaw/services/dev-cloud/.liteasy-data/`
...
1. add JSON store and repositories in `LiteasyClaw/services/dev-cloud/db/`

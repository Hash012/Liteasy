...
The feature must still follow the intent-native generative UI architecture in `project-docs/superpowers/specs/2026-07-05-liteasyclaw-intent-native-generative-ui-architecture-rendered.html`: the model plans structured actions, deterministic code validates and executes them, and UI feedback is generated through the workbench overlay / UI DSL path.

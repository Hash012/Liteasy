...
**Architecture:** This milestone keeps the existing desktop app and deployable demo cloud baseline, but improves the live-demo path end-to-end: login, organization loading, recommendation flow, assistant answers, and artifact entry points. Structural cleanup is limited to high-value boundary splits, especially in `LiteasyClaw/services/dev-cloud/server.mjs`, so the roadshow baseline becomes easier to maintain without losing momentum.

**Tech Stack:** Existing `desktop` stack, existing `LiteasyClaw/services/dev-cloud` Node service, current `Vitest` and Node test runner suites
...
- `LiteasyClaw/services/dev-cloud/server.mjs` is too large and mixes configuration, payload construction, admin HTML, routing, and CLI startup.
- `LiteasyClaw/desktop/src/app/layout/AppShell.tsx` should not absorb additional orchestration logic without extracting helpers.
- `LiteasyClaw/desktop/src/tests/AppShell.test.tsx` should not continue absorbing unrelated scenarios if focused topic-level test files can be introduced instead.

## File Responsibilities

- `LiteasyClaw/services/dev-cloud/server.mjs`: should become a thinner composition/root file.
- `LiteasyClaw/services/dev-cloud/config.mjs`: should own deploy/runtime config parsing.
- `LiteasyClaw/services/dev-cloud/payloads/*.mjs`: should own demo response builders by domain.
- `LiteasyClaw/services/dev-cloud/adminConsole.mjs`: should own admin HTML rendering.
- `LiteasyClaw/services/dev-cloud/requestHandler.mjs`: should own route dispatch wiring if the split is justified by the current code shape.
- `LiteasyClaw/desktop/src/app/layout/AppShell.tsx`: should remain the assembly layer, not accumulate more domain behavior.
- `LiteasyClaw/desktop/src/tests/*`: roadshow-critical tests should be added or split by concern when that reduces pressure on the monolithic `AppShell.test.tsx`.
...
1. Split `LiteasyClaw/services/dev-cloud/server.mjs` by responsibility with tests kept green.
...
- `LiteasyClaw/services/dev-cloud/server.mjs` no longer remains a 1000+ line mixed-responsibility file

...
export function AnnotationApp() {
  ...
  const inbox = usePollingRemote(
    () => session ? api.conversations() : Promise.resolve({ conversations: [] as ConversationSummary[] }),
  ...
  useEffect(() => {
    ...
    void api.consumeAnnotationHandoff(handoffId).then((result) => {
      if (!active) return;
  ...
  return <header className="annotation-header">
    <div className="header-primary">
      ...
      <div className="annotation-account">
        ...
        <Tooltip content="发布批注" relationship="label"><Button appearance="primary" className="publish-action" icon={<Add20Regular />} aria-label="发布批注" onClick={onPublish}>发布批注</Button></Tooltip>
      </div>
    </div>
    <nav aria-label="主要导航">
      <button className={view === "plaza" ? "active" : ""} aria-current={view === "plaza" ? "page" : undefined} onClick={() => onView("plaza")}><Globe20Regular /><span>广场</span></button>
      <button className={view === "following" ? "active" : ""} aria-current={view === "following" ? "page" : undefined} onClick={() => onView("following")}><PersonHeart20Regular /><span>关注</span></button>
      <button className={view === "messages" ? "active" : ""} aria-current={view === "messages" ? "page" : undefined} onClick={() => onView("messages")}><ChatMultiple20Regular /><span>私聊</span>{unreadMessages > 0 && <Badge appearance="filled" color="brand" size="small" aria-label={`${unreadMessages} 条未读消息`}>{unreadMessages > 99 ? "99+" : unreadMessages}</Badge>}</button>
      <button className={view === "mine" ? "active" : ""} aria-current={view === "mine" ? "page" : undefined} onClick={() => onView("mine")}><Library20Regular /><span>我的批注</span></button>
      <button className={view === "organizations" ? "active" : ""} aria-current={view === "organizations" ? "page" : undefined} onClick={() => onView("organizations")}><PeopleTeam20Regular /><span>组织批注</span></button>
    </nav>
  </header>;
}
...
function Plaza({ filters, onCompose, onConversation, onFilters, refresh, session }: {
  ...
  const { data, error } = useRemote(() => api.plaza(filters), key);
  ...
  return <div className="plaza-layout">
    ...
    <aside className="plaza-side">
      <span className="rail-eyebrow">当前视图</span><h2>{filters.sort === "latest" ? "最新发布" : "为你推荐"}</h2>
      <dl>
        {filters.institution && <div><dt>机构</dt><dd>{filters.institution}</dd></div>}
        {filters.documentType && <div><dt>文献类型</dt><dd>{filters.documentType}</dd></div>}
        {filters.educationStage && <div><dt>学段</dt><dd>{filters.educationStage}</dd></div>}
        {filters.query && <div><dt>{filters.query.startsWith("/") ? "动态分类" : "检索"}</dt><dd>{filters.query.replace(/^\//, "")}</dd></div>}
      </dl>
      {filterCount > 0 && <Button appearance="subtle" icon={<ArrowReset20Regular />} onClick={clearFilters}>清除筛选</Button>}
    </aside>
  </div>;
}
...
function AnnotationCard({ annotation, onCompose, onConversation, session }: {
  ...
  async function rate(value: number) {
    try {
      const result = await api.rateAnnotation(current.id, value);
  ...
  async function save() {
    try { const result = await api.saveAnnotation(current.id); setCurrent({ ...current, viewerSaved: result.saved }); }
    catch (reason) { setStatus(reason instanceof Error ? reason.message : "收藏失败"); }
  }
  async function follow() {
    try { const result = await api.followUser(current.author.id); setStatus(result.following ? (result.mutual ? "已互相关注" : "已关注") : "已取消关注"); }
    catch (reason) { setStatus(reason instanceof Error ? reason.message : "关注失败"); }
  }
  async function message() {
    try {
      const result = await api.createConversation(current.author.id);
  ...
  async function moderateOrganization() {
    ...
    try {
      await api.moderateOrganizationAnnotation(current.id, { action: moderationAction, reason: moderationReason.trim() });
  ...
  return <article className={`annotation-card${current.withdrawnAt ? " withdrawn" : ""}`}>
    ...
    <footer>
      ...
      {current.viewerIsAuthor && !current.withdrawnAt && <><Tooltip content="编辑" relationship="label"><Button appearance="subtle" icon={<Edit20Regular />} aria-label="编辑批注" onClick={() => onCompose({ edit: current })} /></Tooltip><Tooltip content="撤回" relationship="label"><Button appearance="subtle" icon={<Delete20Regular />} aria-label="撤回批注" onClick={() => void api.withdrawAnnotation(current.id).then(() => setStatus("已撤回"))} /></Tooltip></>}
...
function AnnotationDetail({ annotationId, onCompose, onConversation, refresh, session }: {
  ...
  const { data, error } = useRemote(() => api.annotation(annotationId), `${annotationId}:${refresh}`);
...
function ReplyThread({ annotation, onCompose, session }: { annotation: CommunityAnnotation; onCompose: (value: { replyTo?: CommunityAnnotation }) => void; session: IdentitySession | null }) {
  const { data, error } = useRemote(() => api.replies(annotation.id), annotation.id);
...
function ReplyItem({ reply }: { onCompose: (value: { replyTo?: CommunityAnnotation }) => void; reply: CommunityReply; session: IdentitySession | null }) {
  ...
  async function save() {
    try {
      const result = await api.updateReply(reply.id, { body });
...
function AnnotationComposer({ context, onClose, onSaved }: {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (original) await api.updateAnnotation(original.id, input);
      else if (parent) await api.createReply(parent.id, { body, shareToPlaza, tags, targets });
      else await api.createAnnotation(input);
      onSaved();
    } catch (reason) {
...
function FollowingAnnotations({ onCompose, onConversation, refresh, session }: {
  ...
  const { data, error } = useRemote(api.followingAnnotations, String(refresh));
...
function MyAnnotations({ onCompose, refresh, session }: { onCompose: (value: { edit?: CommunityAnnotation; replyTo?: CommunityAnnotation }) => void; refresh: number; session: IdentitySession }) {
  const { data, error } = useRemote(api.myAnnotations, String(refresh));
...
function OrganizationAnnotations({ onCompose, refresh, session }: { onCompose: (value: { edit?: CommunityAnnotation; replyTo?: CommunityAnnotation }) => void; refresh: number; session: IdentitySession }) {
  const { data, error } = useRemote(api.organizationAnnotations, String(refresh));
...
function ProfileEditor({ refresh }: { refresh: number }) {
  const { data, error } = useRemote(api.academicProfile, String(refresh));
...
function ProfileForm({ profile }: { profile: AcademicProfile }) {
  ...
  async function save(event: FormEvent) {
    event.preventDefault();
    try { await api.updateAcademicProfile({ educationStage: educationStage || null, institutions }); setStatus("已保存"); }
...
function ConversationDrawer({ conversation, onClose, onInboxChange, session }: { conversation: ConversationSelection; onClose: () => void; onInboxChange: () => void; session: IdentitySession }) {
  ...
  const { data, error } = usePollingRemote(() => api.messages(conversation.id), `${conversation.id}-${refresh}`, 3_000);
  const latestMessageId = data?.messages[data.messages.length - 1]?.id ?? null;
  useEffect(() => {
    if (!latestMessageId) return;
    messageEnd.current?.scrollIntoView({ behavior: "smooth", block: "end" });
    void api.markConversationRead(conversation.id, latestMessageId).then(onInboxChange).catch(() => undefined);
  }, [conversation.id, latestMessageId]);
  async function send(event: FormEvent) {
    event.preventDefault();
    if (conversation.canSend === false) return;
    try {
      await api.sendMessage(conversation.id, inviteOpen ? { body, invitation: { organizationId, role }, kind: "organization_invitation" } : { body, kind: "text" });

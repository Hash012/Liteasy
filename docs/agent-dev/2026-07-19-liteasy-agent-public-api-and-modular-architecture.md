...
Artifact modal
  ...
  → development/test-data/agent-results/<artifactId>.json
  → ArtifactTabs
```
...
```bash
git add development/test-data/agent-results/*.json
git commit -m "docs: add agent analysis result"
git push
```

本地模型和测试 API 配置统一写入已被 Git 忽略的 `development/dev-cloud/.env.local`，不得与结果一起提交。
...
- 版本化结果写入 `development/test-data/agent-results/`，可经人工检查后 Git 提交并由其他开发者查看。
...
无需安装新依赖。在 `products/liteasy/apps/desktop` 执行：

```bash
npm run dev:test-api
```

脚本只在进程内读取 `development/dev-cloud/.env.local` 的 `OPENAI_API_KEY`、`OPENAI_BASE_URL` 和模型配置，不打印或复制密钥。`VITE_LITEASY_OPENAI_MODEL` 可覆盖桌面开发模型标识。端口冲突时，现有 `dev-with-cloud.mjs` 会自动选择后续可用端口，并把实际 dev-cloud endpoint 注入 Vite；应以终端启动日志为准。

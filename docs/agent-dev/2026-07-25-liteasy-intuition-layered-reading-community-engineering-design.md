...
```text
development/dev-cloud npm test
  -> 60 / 60 passed

products/liteasy/apps/desktop Agent 核心定向测试
  -> 167 / 168 passed
  -> 唯一失败：测试期待 artifact.generate，实际 capability summary 为 artifact-generate

products/liteasy/apps/desktop npm test
  -> 564 / 648 passed
  -> 16 个 test files 失败
  -> 部分失败来自 .env.local 覆盖测试模型/端口，以及已经变化的 UI 文案和结构断言

products/liteasy/apps/desktop npm run build
  -> failed
...
development/test-data/agent-results/*.json
...
```text
products/liteasy/apps/desktop/src/app/features/layered-reading/
...
products/liteasy/apps/desktop/src/app/features/intuition-graph/
...
products/liteasy/apps/desktop/src/app/features/intuition-community/
...
products/liteasy/apps/desktop/src/app/features/personalization/
...
- `products/liteasy/apps/desktop npm run build` 通过；

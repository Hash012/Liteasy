...
- 本地 PDF.js 读取首页和 Info/XMP 标题。首页无可选文本或读取失败时，使用已有导入流程得到的首页文本（包括成功的 OCR）。内嵌标题必须在首页得到验证，才作为标题搜索线索。
- 提取 DOI/arXiv ID；默认沿用设置中的 Crossref，也支持现有 OpenAlex、Semantic Scholar 和云端元数据服务。直接服务模式的 arXiv ID 查询走[官方 Atom API](https://info.arxiv.org/help/api/user-manual.html)，串行且请求间隔至少三秒，不转发其他服务的密钥，保留明确版本号。
- 无 ID 时，以最多 350 个字符的首页书目信息检索。自动接受要求候选题名完整出现在首页摘要之前；标题搜索还要求作者姓氏一致且只有一个匹配候选。明确的 arXiv ID/版本必须一致；冲突和歧义不自动接受。DOI 无结果或指向其他题名时再尝试标题搜索。
...
主要实现：`features/metadata/pdfRecognition.ts`、`controllers/usePdfMetadataImportController.ts`、`features/paper-services/arxivMetadata.ts`。AppShell 仅组合控制器；未复制 Zotero 的 AGPL 源码，也未调用其 recognizer 服务。

## 边界与验证

这是独立实现的保守书目识别流程，并非 Zotero recognizer 的完整复刻。暂不实现 ISBN/书籍识别、全文服务端版面推断、可编辑命名模板、批量撤销和元数据修改后的持续改名。扫描件依赖现有 OCR 成功；全文抽取失败时不会进入自动元数据识别。少于 15 个归一化字符的短标题、复杂数学标题或排版使标题无法连续核对时保留原名，可继续使用现有手动文献身份检索入口。网络请求仅发送 ID 或有界书目搜索片段，不发送 PDF 文件。
...
本次验证：相关单元与集成测试通过，桌面 `npm run build`（包含 TypeScript、资源验证和生产资源检查）通过。公开接口抽查中，Crossref 的 `10.1038/nphys1170` 返回 HTTP 200；arXiv 的 `1706.03762v5` 请求返回 HTTP 429，因此未把 arXiv 线上可用性视为已验证。此时应用保留文件并提示，可稍后从现有文献身份入口重新查询。

...
**Tech Stack:** Existing `desktop` stack, existing `services/dev-cloud` Node service, current `Vitest` and Node test runner suites
...
- `services/dev-cloud/server.mjs` is too large and mixes configuration, payload construction, admin HTML, routing, and CLI startup.
- `desktop/src/app/layout/AppShell.tsx` should not absorb additional orchestration logic without extracting helpers.
- `desktop/src/tests/AppShell.test.tsx` should not continue absorbing unrelated scenarios if focused topic-level test files can be introduced instead.

## File Responsibilities

- `services/dev-cloud/server.mjs`: should become a thinner composition/root file.
- `services/dev-cloud/config.mjs`: should own deploy/runtime config parsing.
- `services/dev-cloud/payloads/*.mjs`: should own demo response builders by domain.
- `services/dev-cloud/adminConsole.mjs`: should own admin HTML rendering.
- `services/dev-cloud/requestHandler.mjs`: should own route dispatch wiring if the split is justified by the current code shape.
- `desktop/src/app/layout/AppShell.tsx`: should remain the assembly layer, not accumulate more domain behavior.
- `desktop/src/tests/*`: roadshow-critical tests should be added or split by concern when that reduces pressure on the monolithic `AppShell.test.tsx`.
...
1. Split `services/dev-cloud/server.mjs` by responsibility with tests kept green.
...
- `services/dev-cloud/server.mjs` no longer remains a 1000+ line mixed-responsibility file

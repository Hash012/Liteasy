...
> **Execution amendment (2026-08-10):** The initial real PostgreSQL run,
> after the Task 2 test was written, exposed two repository defects that
> require files outside this task's original three-file list. The required
> `createVisualizationTestPool({ connectionString, sslMode: "disable" })`
> interface reached `createPostgresPool()` as a TLS configuration and failed
> with `DEPTH_ZERO_SELF_SIGNED_CERT`; authorize

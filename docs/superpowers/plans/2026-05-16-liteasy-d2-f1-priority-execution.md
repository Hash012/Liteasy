...
**Architecture:** This plan is an execution coordinator, not a replacement for the detailed SaaS plan docs. It sequences the already-written D2 and M1 work against the current repository state: `services/dev-cloud/server.mjs` still carries mixed responsibilities at 900+ lines, `desktop` still boots from seeded demo papers instead of a file-backed local library root, and the resource-scope modules described by M1 do not yet exist in code.
...
- `services/dev-cloud/server.mjs`: `953` lines
- `desktop/src/app/layout/useAppShellStores.ts`: still seeds `starterPapers` into the workspace store on startup
- `desktop/src-tauri/src/main.rs`: only registers `import::mock_import`
- `desktop/src/app/features/resources/`: does not exist yet
- verification baseline:
  - `node --test services/dev-cloud/server.test.mjs services/dev-cloud/providers/openaiResponses.test.mjs`: PASS
...
```bash
wc -l services/dev-cloud/server.mjs
```
...
rg -n "starterPapers|mock_import|generate_handler|createWorkspaceStore|features/resources" \
  desktop/src/app/layout/useAppShellStores.ts \
...
- services/dev-cloud/payloads/*.mjs
- services/dev-cloud/README.md
- docs/qa/roadshow-demo-guide.md
- docs/qa/environment-startup-guide.md

F1 write scope:
- desktop/src/app/features/resources/*
- desktop/src/app/features/library/*
- desktop/src/app/features/workspace/*
...
- Create: `services/dev-cloud/requestHandler.mjs`
- Create: `services/dev-cloud/payloads/*.mjs`
- Modify: `services/dev-cloud/server.test.mjs`
- Modify if needed: `services/dev-cloud/README.md`
...
```text
services/dev-cloud/payloads/policyPayloads.mjs
...
services/dev-cloud/payloads/modelPayloads.mjs
```
...
Keep `services/dev-cloud/server.mjs` responsible only for:
...
**Files:**
- Modify if needed: `desktop/src/app/features/network/cloudErrorMessage.ts`
- Modify if needed: `desktop/src/app/features/organization/useOrganizationWorkspace.ts`
- Modify if needed: `desktop/src/tests/cloudErrorMessage.test.ts`
- Modify if needed: `desktop/src/tests/useOrganizationWorkspace.test.ts`
- Modify only if unavoidable: `desktop/src/tests/AppShell.test.tsx`
- Modify if behavior changes: `docs/qa/roadshow-demo-guide.md`
...
```bash
cd desktop && npm test -- src/tests/cloudErrorMessage.test.ts src/tests/useOrganizationWorkspace.test.ts src/tests/AppShell.test.tsx
```
...
- Create: `desktop/src/tests/resourceActionPolicy.test.ts`
...
```bash
cd desktop && npm test -- src/tests/resourceActionPolicy.test.ts src/tests/localLibraryClient.test.ts
```
...
- Modify: `desktop/src/app/layout/useAppShellStores.ts`
...
- `node --test services/dev-cloud/server.test.mjs services/dev-cloud/providers/openaiResponses.test.mjs` passes

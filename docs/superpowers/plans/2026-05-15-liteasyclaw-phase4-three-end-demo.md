...
- `services/dev-cloud/server.mjs`: Owns local dev-cloud HTTP routes, admin HTML shell, and admin JSON payloads.
...
Add this test to `services/dev-cloud/server.test.mjs` after the root index test:
...
In `services/dev-cloud/server.mjs`, add a `writeHtml(request, response, statusCode, html)` helper next to `writeJson`, then add this route before the final method/path fallback:
...
Add `GET /v1/admin/governance-dashboard` to `services/dev-cloud/server.mjs`. Return deterministic JSON:
...
1. 启动开发云：`node /home/octopus/Liteasy/services/dev-cloud/server.mjs`
2. 启动桌面前端：`cd /home/octopus/Liteasy/desktop && npm run dev`
...
- [x] `node --test services/dev-cloud/server.test.mjs services/dev-cloud/providers/openaiResponses.test.mjs` passed with 19 server/provider tests.
- [x] `cd desktop && npm test` passed with 62 desktop test files and 219 tests.
- [x] `cd desktop && npm run build` passed through TypeScript and Vite production build.
...
- [x] Verification: `node --test services/dev-cloud/server.test.mjs services/dev-cloud/providers/openaiResponses.test.mjs` passed with 20 server/provider tests.
- [x] Verification: `cd desktop && npm test && npm run build` passed with 62 desktop test files, 219 desktop tests, and a successful production build.
...
- [x] Verification: `node --test services/dev-cloud/server.test.mjs services/dev-cloud/providers/openaiResponses.test.mjs` passed with 21 server/provider tests.
- [x] Verification: `cd desktop && npm test` passed with 63 desktop test files and 223 tests.
- [x] Verification: `cd desktop && npm run build` passed through TypeScript and Vite production build.
...
- [x] Verification: `node --test services/dev-cloud/server.test.mjs services/dev-cloud/providers/openaiResponses.test.mjs` passed with 22 server/provider tests.
- [x] Verification: `cd desktop && npm test && npm run build` passed with 63 desktop test files, 223 desktop tests, and a successful production build.

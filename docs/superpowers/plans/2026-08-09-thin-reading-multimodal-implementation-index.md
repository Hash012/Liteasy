...
- Real isolated PostgreSQL verification passed after applying all 26 migrations: the repository-wide integration verifier reported `verified: true`, and the visualization PostgreSQL suite passed 3/3 with zero skips. The latter covers governance transactions, grouped structured/image reservations under one concurrency slot, atomic two-reservation settlement/publication, stale-route rollback, provider accounting, and durable generation requests.

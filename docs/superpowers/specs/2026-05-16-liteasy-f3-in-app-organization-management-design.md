...
- organization entry UI in `desktop/src/app/features/organization/`
...
`desktop/src/app/features/organization/organization.types.ts` must become the single shared contract for:
...
`services/dev-cloud/` should formalize organization actions without collapsing them into unrelated modules.

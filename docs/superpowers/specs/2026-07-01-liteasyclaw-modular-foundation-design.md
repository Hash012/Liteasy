...
`products/liteasy/apps/desktop/src/app/layout/AppShell.tsx` currently composes most feature hooks directly. It is the natural place for every new feature to be wired, which makes it a merge-conflict hotspot.
...
`development/dev-cloud/requestHandler.mjs` is already better than the original server, but it still owns all route dispatch. `server.test.mjs` is also a shared hotspot.
...
```text
products/liteasy/apps/desktop/src/app/shell/
```

Migration source:

- `products/liteasy/apps/desktop/src/app/layout/*`
...
Add a short `products/liteasy/apps/desktop/src/app/README.md` or `docs/engineering/module-boundaries.md`.

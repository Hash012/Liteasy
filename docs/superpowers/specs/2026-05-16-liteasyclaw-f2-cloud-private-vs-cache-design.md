...
- `products/liteasy/apps/desktop/src/app/features/collection/` already has a cloud collection client/runtime path, but `useCollectionItems` still loads and stores collection items through local browser storage.
- `products/liteasy/apps/desktop/src/app/features/recommendations/` still treats recommendation cache as a frontend-only in-memory `Map` inside `useRecommendations`.
- `development/dev-cloud/` already exposes:
...
`products/liteasy/apps/desktop/src/app/features/collection/` remains the user-cloud-private favorites domain.
...
`products/liteasy/apps/desktop/src/app/features/recommendations/` becomes the recommendation generation plus recommendation-cache orchestration domain.
...
`products/liteasy/apps/desktop/src/app/features/library/LibraryPane.tsx` keeps the current user-facing section names and drag behavior.
...
`development/dev-cloud/` must separate:
...
1. add JSON store and repositories in `development/dev-cloud/db/`

...
后续产品要求见[统一资源文件系统规格第 1.2、8.3–8.7 与 12.3 节](2026-09-14-unified-resource-filesystem-spec.md#12-核心产品目标围绕正在使用的内容与-ai-协作)：单条批注、收藏网页、批量批注、设置文件和混合资源均须支持就地提问、review 与按能力受控修改。统一引用和聊天附件只是基础，交付还须包含来源定位、逐项变更采纳、原处更新及冲突/撤销闭环。其 F0–F3 是后续提案分期，不据此将这些新增场景认定为本规格 P0 已实现。

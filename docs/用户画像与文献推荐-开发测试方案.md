...
> 制定时间：2026-08-03
> 配套总纲：`project-docs/Saas/2026-08-03-用户画像与文献推荐-开发计划与数据模型.md`
...
| 文件 | 动作 | 说明 |
|---|---|---|
| `LiteasyClaw/services/dev-cloud/db/migrations/003_works_and_identity.sql` | 🆕 | `works`、`work_identifiers`、`citation_edges` 三表 |
| `LiteasyClaw/services/dev-cloud/db/workRepository.mjs` | 🆕 | SQLite 仓库：`resolveWork`、`getWork`、`listIdentifiers`、`addCitationEdge` |
| `LiteasyClaw/services/dev-cloud/payloads/identityResolutionPayloads.mjs` | 🆕 | 请求校验 + 公共快照构建（`buildWorkResolutionPayload`） |
| `LiteasyClaw/services/dev-cloud/db/workRepository.test.mjs` | 🆕 | 仓库单测：幂等、多标识合并、版本关系、引用边 |
| `LiteasyClaw/services/dev-cloud/payloads/identityResolutionPayloads.test.mjs` | 🆕 | 校验/快照单测 |
| `LiteasyClaw/services/dev-cloud/server.test.mjs` | ✏️ | 新增 `POST /v1/works/resolve` 端点集成测试 |
| `LiteasyClaw/services/dev-cloud/requestHandler.mjs` | ✏️ | 注册端点 + `availableEndpoints` + 注入 `workRepository` |
...
每模块后：
```bash
cd LiteasyClaw/services/dev-cloud && npm test          # node --test
cd LiteasyClaw/desktop && npm run build && npm test    # 提交前
```

...
**Architecture:** This plan keeps the existing `desktop` app and current `development/dev-cloud` service shape, but hardens them into a deployment-oriented demo baseline. It focuses on environment configuration, deployability, server start paths, smoke checks, presentation-safe defaults, and documentation, while avoiding work that would be thrown away immediately after the roadshow.

**Tech Stack:** Existing `desktop` app, existing `development/dev-cloud` Node service, deployment scripts and environment configuration, existing QA docs
...
- `development/dev-cloud/server.mjs`: must support explicit deploy-time configuration and stable host/port/environment behavior.
- `development/dev-cloud/README.md`: must explain deployment/start/run behavior for local and cloud demo use.
- `README.md`: must link to the roadshow deployment flow and clarify the three-end demo shape.
- `docs/qa/environment-startup-guide.md`: must include a roadshow deployment/start checklist.
...
```bash
git add development/dev-cloud/server.mjs development/dev-cloud/server.test.mjs
